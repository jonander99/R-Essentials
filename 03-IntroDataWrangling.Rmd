...
Here we save the the modified dataset as a new dataframe called gap97 by using an assignment arrow "<-" to save the resulting pipeline to gap97.
```{r,tidy=FALSE}
gap97 <- gapminder %>% 
  filter(continent!="Oceania" & year==1997) 
#
...
# count rows with missing values for variable y
tempdf %>%
  dplyr::tally(is.na(y))
...
```{r}
tempdf %>%
  filter(!is.na(x),  # allow obs with nonmissing x
         !is.na(y),  # allow obs with nonmissing y
         !is.na(z))  # allow obs with nonmissing z
```
