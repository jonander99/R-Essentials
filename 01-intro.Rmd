...
### CSV File Located in Working Directory
...
The *readr* package inside the *tidyverse* family of packages has a slightly nicer read function you should know about.  We use the `readr::` prefix to inform readers that the `read_csv` function resides in the *readr* package.  This read function will not auto-convert character variables to category/factor variables.

```{r,eval=FALSE}
dataframe <- readr::read_csv("file.csv",col_names = TRUE)
```

### Read Data From a URL Web Location

If data is located in an accessible location on the internet, you can simply add the location pathway or URL address to the *read.csv* command to access the data.
...
Here is a similar command using the *readr* package.

```{r}
#import data from URL
data3 <- data.table::fread('https://raw.githubusercontent.com/Statology/Miscellaneous/main/basketball_data.csv')
...
Reading data directly from excel spreadsheets is more complex and you should read documentation for the *readxl* package.  Here we illustrate the basic idea.

```{r,echo=TRUE,eval=FALSE}
library(readxl)
# Read sheet names 'sheet_name' from excel file
df = read_excel("/Users/new_file.xlsx", sheet='sheet_name')
#or
# Read sheet 3 from excel file
df = read_excel("/Users/new_file.xlsx", sheet=3)
```
